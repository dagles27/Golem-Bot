...
export default function GolemDashboard() {
 ...
 const akter, setCharakter] = useState('aggressiv');
 const , setEmotion] = useState({ mood: 'neutral', message: 'Bereit für Action!' });

 const ades, setTrades] = useState();
 ...
 const newTrades = trades, { profit: neuerProfit }].slice(-5);
 setTrades(newTrades);
 ...
 {Object.entries(skills).map((, wert]) => (
